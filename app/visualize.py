...
class AlligatorVisualization:
    def __init__(self, ohlc_ocs, tf):
        self.fig = plotly.tools.make_subplots(rows=4, cols=1, specs=[[{'rowspan': 3}], [None], [None], [{}]],
    ...
    def plot_fractals(self):
        ...
        self.fig.add_trace(go.Scatter(mode="markers", x=fractal_high['time_str'], y=fractal_high['High'],
                                      marker_symbol='triangle-down',
                                      marker_line_color="midnightblue", marker_color="red", marker_size=7,
                                      name='aims high'))

        fractal_low = self.data.loc[self.data['fractals_low'] == True]
        self.fig.add_trace(
            go.Scatter(mode="markers", x=fractal_low['time_str'], y=fractal_low['Low'], marker_symbol='triangle-up',
    ...
    def plot_ao_indicator(self):
        self.fig.append_trace(go.Bar(x=self.data['time_str'],
                                     y=self.data['ao'],
                                     marker=dict(
                                         color=list(map(self.set_color, self.data['ao'], self.data['ao_diff']))),
                                     name='AO oscillator'
                                     ),
                              row=4, col=1)
